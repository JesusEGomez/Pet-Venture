...
export async function welcomeEmail(email, displayName) {
  try {
    const info = await transporter.sendMail({
      ...
      subject: "Bienvenido a PetVenture", // Subject line
      // text: "Hello world?", // plain text body
      html: `
			<h2> Bienvenido a PetVenture ${displayName} </h2>
			<h3> Estamos muy felices que nos hayas escogidos para tus futuras compras. </h3>
			<h4> Ir a PetVenture clickeando aquí ${homeURL}</h4>
			`,
    });
  ...
  try {
    const info = await transporter.sendMail({
      ...
      html: `
			...
			<h3> Si estas recibiendo esta email es porque tu compra en PetVenture ha sido realizada con exito.</h3>
